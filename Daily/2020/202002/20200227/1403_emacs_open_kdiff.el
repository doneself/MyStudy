...
(defun kdiff-buffers ()
  ...
  (let* ((kdiff-path "C:/Program Files/KDiff3/kdiff3.exe")
		 ...
		 (buffer1 (completing-read "选择buffer(1):" file-buffer-name-list))
		 (buffer2 (completing-read "选择buffer(2):" file-buffer-name-list)))
	(if (and (get-buffer buffer1) (get-buffer buffer2))
		(w32-shell-execute "open" kdiff-path (format " -b %s %s" (buffer-file-name (get-buffer buffer1)) (buffer-file-name (get-buffer buffer2))))
		(message (format "buffer:%s,%s not exist." buffer1 buffer2)))))
